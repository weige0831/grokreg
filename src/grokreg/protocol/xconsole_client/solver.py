    ...
    def solve_turnstile(
        ...
        *,
        ...
        Args:
            ...
            proxy: Optional proxy URL. For the local Camoufox solver this is
                required to keep captcha egress identical to protocol HTTP.
                Remote YesCaptcha Proxyless ignores it (workers use their own IP).
        ...
        if use_premium:
            ...
            # Local solver: when a proxy is provided, prefer TurnstileTask so the
            # task type reflects "with proxy"; still accept Proxyless aliases.
            if local and proxy:
        ...
        for idx, task_type in enumerate(task_types):
            ...
            # Always attach proxy for local solver so Camoufox shares registration egress.
            # Remote YesCaptcha Proxyless workers ignore it; harmless if present.
            if proxy:
                task["proxy"] = proxy
            try:
                self._progress(
                    ...
                    + (" proxy=yes" if proxy else " proxy=no")
                )
