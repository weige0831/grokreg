...
# Browser binaries: download on demand into cache (no hard-coded version).
# Cache defaults to ../data/register_lite/cache when that data dir exists.
ROOT_DIR="$(cd .. && pwd)"
if [[ -z "${XDG_CACHE_HOME:-}" ]]; then
  if [[ -d "${ROOT_DIR}/data/register_lite" ]]; then
    export XDG_CACHE_HOME="${ROOT_DIR}/data/register_lite/cache"
  elif [[ -d "${ROOT_DIR}/generated/register_lite" ]]; then
    export XDG_CACHE_HOME="${ROOT_DIR}/generated/register_lite/cache"
  fi
fi
